...
function getQ(cat: string): Question | null {
  const f = allQuestions.filter(q => q.category === cat);
  return f.length > 0 ? f[Math.floor(Math.random() * f.length)] : null;
}
...
export default function ChallengeGamePage() {
  ...
  const handleAnswer = (idx: number) => {
    ...
    const timeMs = Math.min(Date.now() - (game.state?.questionStartTime || Date.now()), 15000);

    setTimeout(() => {
      game.submitAnswer(correct, timeMs);
      setSelected(null);
      setShowExp(false);
    }, correct ? 1200 : 2000);
  };
