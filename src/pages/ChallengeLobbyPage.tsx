...
import {
  ...
  Hourglass, Check, Send, MessageSquare, X, Hash, Wifi, WifiOff
} from "lucide-react";
...
export default function ChallengeLobbyPage() {
  ...
  // Presencia: WebSocket (multi-navegador/dispositivo) con fallback localStorage
  const localPlayers = useOnlinePlayers(user?.id || 0, user?.name || "Jugador");
  const { messages: globalMessages, send: sendGlobal, connected: wsConnected, onlineUsers: wsOnlineUsers } =
    useWebSocketChat("global", user?.id || 0, user?.name || "Jugador");
  const onlinePlayers = wsConnected
    ? wsOnlineUsers.filter((u: any) => u.id !== (user?.id || 0))
    : localPlayers;

  // Public and my challenges (tRPC)
  ...
  const roomCodeOf = (d: any) => d.roomCode || d.syncCode || "";
  ...
  const handleCrearSala = async () => {
    ...
    try {
      ...
      if (result?.id) navigate(`/challenge/${result.id}`);
    } catch (e: any) {
  ...
  const handleAceptar = async (challengeId: number) => {
    try { await acceptMut.mutateAsync({ challengeId }); navigate(`/challenge/${challengeId}`); }
  ...
  const handleUnirse = async (challengeId: number) => {
    try { await joinMut.mutateAsync({ challengeId }); navigate(`/challenge/${challengeId}`); }
    catch (e: any) { alert(e.message || "Error"); }
  };

  const handleJugar = (id: number) => { navigate(`/challenge/${id}`); };

  const handleJoinByCode = async () => {
    setJoinError("");
    const code = syncCode.trim().toUpperCase();
    if (code.length !== 6) { setJoinError("Codigo invalido (6 caracteres)"); return; }
    const result = await joinByCode.join(code);
    if (result?.id) {
      setSyncCode("");
      setShowJoinCode(false);
      navigate(`/challenge/${result.id}`);
    } else {
      setJoinError("Codigo no encontrado o sala llena");
    }
  };
      ...
      <div className="flex-1 flex flex-col">
        <div className="max-w-3xl mx-auto w-full px-6 py-6">
          ...
          <div className="bg-gradient-to-r from-amber-500/20 to-amber-600/10 rounded-2xl p-4 border border-amber-500/20 mb-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1 ${wsConnected ? "bg-blue-500/20 text-blue-400" : "bg-yellow-500/20 text-yellow-400"}`}>
                  {wsConnected ? <><Wifi className="w-3 h-3" />WS</> : <><WifiOff className="w-3 h-3" />Offline</>}
                </span>
                <span className="text-xs text-white/50">Activos: {activeDuels.length}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            {showJoinCode ? (
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <label className="text-sm font-bold text-white/70 mb-2 block">Codigo de sala (6 caracteres)</label>
                <div className="flex gap-2">
                  <input type="text" value={syncCode} onChange={e => setSyncCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 6))} placeholder="ABC123" maxLength={6}
                    className="flex-1 px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-amber-500/50 text-center tracking-widest font-mono text-lg" />
                  <button onClick={handleJoinByCode} disabled={joinByCode.isPending}
                    ...
                    <ArrowRight className="w-4 h-4" /> {joinByCode.isPending ? "..." : "Unirse"}
                  </button>
                </div>
                ...
                <div key={d.id} className="bg-white/5 rounded-xl p-4 border border-white/10">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-white/50">Por: {d.challengerName} | #{d.id} | Codigo: <span className="text-amber-400 font-mono">{roomCodeOf(d)}</span></p>
                    </div>
          ...
          {tab === "players" && (
            <div className="space-y-3">
              {onlinePlayers.length === 0 ? (
                <div className="text-center py-12 text-white/40"><Users className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>No hay jugadores en linea</p><p className="text-xs mt-1">Los jugadores apareceran cuando se conecten</p></div>
              ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-sm flex items-center gap-1">
                            ...
                            <span className="text-[10px] px-1.5 py-0.5 bg-green-500/20 text-green-400 rounded-full">En linea</span>
                          </p>
                        </div>
                      </div>
                ...
                <div className="mt-4 space-y-2">
                  ...
                  {incomingChallenges.map((d: any) => (
                    <div key={d.id} className="bg-green-500/10 rounded-xl p-4 border border-green-500/20">
                      <p className="font-bold text-sm mb-2">{d.challengerName} te reto</p>
                      <div className="flex gap-2">
                      ...
                      <button onClick={() => handleJugar(d.id)} className="block w-full text-left bg-gradient-to-r from-amber-500/10 to-transparent rounded-xl p-4 border border-amber-500/20 hover:bg-amber-500/20 transition">
                        <div className="flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-xs text-white/50">#{d.id} | Codigo: <span className="text-amber-400 font-mono">{roomCodeOf(d)}</span></p>
                          </div>
